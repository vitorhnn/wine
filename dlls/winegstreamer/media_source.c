...
static HRESULT media_stream_constructor(struct media_source *source, GstPad *pad, DWORD stream_id, struct media_stream **out_stream)
{
    ...
    media_type = mf_media_type_from_caps(desired_caps);
    if (TRACE_ON(mfplat))
    {
        gchar *caps_str = gst_caps_to_string(caps), *desired_caps_str = gst_caps_to_string(desired_caps);
        TRACE("caps %s desired_caps %s\n", debugstr_a(caps_str), debugstr_a(desired_caps_str));
        g_free(caps_str);
        g_free(desired_caps_str);
    }
