...
struct mf_decoder
{
    ...
    GstPad *input_src, *their_sink/*, *their_src*/;
...
static HRESULT WINAPI mf_decoder_GetInputStreamInfo(IMFTransform *iface, DWORD id, MFT_INPUT_STREAM_INFO *info)
{
    ...
    /* MFT_INPUT_STREAM_DOES_NOT_ADDREF could be set, as we do a full copy right now,
       but we will optimize that in the future */
        ...
        IMFMediaType **type)
{
    ...
    defer {IMFMediaType_Release(output_type);}
...
static void decoder_update_pipeline(struct mf_decoder *This)
{
    ...
    /* tear down current pipeline, this should probably be GST_STATE_NULL */
    ...
    /* don't even bother building it up unless we have an input and output */
    if (!This->input_type || !This->output_type)
        return;

    /* Yes, we do leave a lot of invalid state here in case of failure,
       but it will be cleaned up on the next call */
    if (!(This->input_src = gst_pad_new_from_template(gst_pad_template_new(
        "mf_src",
        GST_PAD_SRC,
        GST_PAD_ALWAYS,
        caps_from_media_type(This->input_type)
        ), "input_src")))
    {
...
/* TODO: handle refcount of IMFSample and destruction of gst_buffer */
static HRESULT WINAPI mf_decoder_ProcessInput(IMFTransform *iface, DWORD id, IMFSample *sample, DWORD flags)
{
    ...
    EnterCriticalSection(&This->state_cs);
    defer {LeaveCriticalSection(&This->state_cs);}
...
HRESULT generic_decoder_construct(REFIID riid, void **obj, enum decoder_type type)
{
    ...
    TRACE("Looking for decoders with input types: %s\n", gst_caps_to_string(input_caps));

    /* For every parser that accepts all the input type, try to find a decoder that accepts the parser src caps */
    /* If different input types need different parsers, this won't work, so we'll need to move this to decoder_update_pipeline */
    ...
    if (!(g_list_length(parser_list_one)))
    {
        ...
        if (!(g_list_length(decoder_list_one)) ||
            !(This->decoder = gst_element_factory_create(g_list_first(decoder_list_one)->data, NULL)))
        {
            gst_plugin_feature_list_free(decoder_list_one);
    ...
    else
    {
        for (walk = (GList *) parser_list_one; walk; walk = g_list_next(walk))
        {
            ...
            for (walk_templ = (GList *)templates; walk_templ; walk_templ = g_list_next(walk_templ))
            {
                ...
                defer {gst_caps_unref(templ_caps);}
                ...
                if (!(This->decoder = gst_element_factory_create(g_list_first(decoder_list_one)->data, NULL)))
                {
                    gst_plugin_feature_list_free(decoder_list_one);
                    ERR("Failed to create decoder\n");
                    hr = ERROR_INTERNAL_ERROR;
    ...
    if (!(This->decoder))
    {
        ERR("Unable to find gstreamer decoder.\n");
        hr = ERROR_INTERNAL_ERROR;
        goto fail;
    }
    ...
    *obj = This;
    ...
    fail:
    ERR("Failed to create Decoder MFT type %u, ", type);
    if (FAILED(hr))
        ERR("hr = %#x\n", hr);
    else
        ERR("ret = %d\n", ret);
    ...
    return ret ? ERROR_INTERNAL_ERROR : hr;
}
